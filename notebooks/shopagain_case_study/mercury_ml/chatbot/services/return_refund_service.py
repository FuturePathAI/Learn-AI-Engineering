...
class ReturnRefundServiceFunctions:
    @staticmethod
    def replace_product(company_id: str,
                        order_id: Optional[int] = None, 
                        email: Optional[str] = None, 
                        mobile: Optional[str] = None, 
                        product_id: Optional[str] = None) -> Tuple[str, str]:
        try:
            ...
            if 'order_status' in order_info:
                if order_info['order_status'].lower() == 'fulfilled':
                    return process_product_replacement(order_id, email, mobile, product_id, order_info), ""
                else:
                    return f"The product cannot be replaced as the order status is {order_info['order_status']}.", ""
            else:
    ...
    @staticmethod
    def return_product(company_id: str,
                       order_id: Optional[int] = None, 
                       email: Optional[str] = None, 
                       mobile: Optional[str] = None, 
                       product_id: Optional[str] = None) -> Tuple[str, str]:
        try:
            ...
            if 'order_status' in order_info:
                if order_info['order_status'].lower() == 'fulfilled':
                    return process_product_return(company_id, order_id, email, mobile, product_id, order_info), ""
                else:
                    return f"The product cannot be returned as the order status is {order_info['order_status']}.", ""
            else:
                return "Order not found.", "Could you please check the provided information and try again?"
        except Exception as e:
            logging.error(f"Exception occurred: {str(e)}")
    ...
    @staticmethod
    def initiate_return_or_refund(company_id: str, order_id: int, action: str) -> Tuple[str, str]:
        """
        ...
        try:
            ...
            # Update the database
            if action == 'return':
                df_orders.loc[df_orders['order_id'] == order_id, 'return_status'] = 'Initiated'
                status_message = f"The return process for Order ID {order_id} has been initiated."
            else:  # action == 'refund'
                df_orders.loc[df_orders['order_id'] == order_id, 'refund_status'] = 'Initiated'
                status_message = f"The refund process for Order ID {order_id} has been initiated."
    ...
    @staticmethod
    def get_return_status(company_id: str, order_id: int = None, email: str = None, phone_number: str = None) -> Tuple[str, str]:
        """Get the return status of an order given its Order ID, email, or phone number.
            Returns: (status_message, next_step)
        """
        try:
            if order_id is not None:
                status = df_orders[df_orders['order_id'] == int(order_id)]['return_status'].iloc[0]
                return f"The return status of your order with Order ID {order_id} is {status}.", ""
            elif email is not None:
                status = df_orders[df_orders['email'] == email]['return_status'].iloc[0]
                return f"The return status of your order for email {email} is {status}.", ""
            elif phone_number is not None:
                status = df_orders[df_orders['phone_number'] == phone_number]['return_status'].iloc[0]
                return f"The return status of your order for phone number {phone_number} is {status}.", ""
            else:
                return "", "Could you please provide the Order ID, email, or phone number?"
        except IndexError:
            return "Order not found.", "Could you please check the provided information and try again?"
    ...
    @staticmethod
    def get_refund_status(company_id: str, order_id: int = None, email: str = None, phone_number: str = None) -> Tuple[str, str]:
        """Get the refund status of an order given its Order ID, email, or phone number.
            Returns: (status_message, next_step)
        """
        try:
            if order_id is not None:
                status = df_orders[df_orders['order_id'] == int(order_id)]['refund_status'].iloc[0]
                return f"The refund status of your order with Order ID {order_id} is {status}.", ""
            elif email is not None:
                status = df_orders[df_orders['email'] == email]['refund_status'].iloc[0]
                return f"The refund status of your order for email {email} is {status}.", ""
            elif phone_number is not None:
                status = df_orders[df_orders['phone_number'] == phone_number]['refund_status'].iloc[0]
                return f"The refund status of your order for phone number {phone_number} is {status}.", ""
            else:
                return "", "Could you please provide the Order ID, email, or phone number?"
        except IndexError:
            return "Order not found.", "Could you please check the provided information and try again?"
    ...
    def get_refund_policy(company_id: str, query: str) -> Tuple[str, str]:
        ...
        try:
            ...
            if response:
                ...
                return "Unable to retrieve the refund policy at the moment.", "Please try again later."
        except Exception as e:
            logging.error(f"Exception occurred: {str(e)}")
            return str(e), "An error occurred. Could you please try again?"
