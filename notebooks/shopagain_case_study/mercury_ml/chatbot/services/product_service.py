...
from mercury_ml.chatbot.apis.shopagain import fetch_product_info
...
class ProductServiceFunctions:
    ...
    def get_product_info(company_id: str, product_id: str = None) -> Tuple[str, str]:
        try:
            ...
            if 'error' in product_info:
                return product_info['error'], "An error occurred. Could you please try again?"
            
            if not product_info:
                return f"No information found for Product ID {product_id}.", "Would you like to search for another product?"
    ...
    @staticmethod
    def get_product_variants(company_id: str, product_id: str = None) -> Tuple[str, str]:
        try:
            if product_id is None:
                return "", "Could you please provide the Product ID?"
            ...
            return "Other variants available for Product ID " + str(product_id) + ":\n" + '\n'.join(variant_details), ""
        except Exception as e:
            logging.error(f"Exception occurred: {str(e)}")
            return str(e), "An error occurred. Could you please try again?"
    ...
    def get_product_vendor(company_id: str, product_id: str = None) -> Tuple[str, str]:
        try:
            ...
            return f"The product with Product ID {product_id} is available from Vendor {product_info['vendor']}.", ""
        except Exception as e:
            logging.error(f"Exception occurred: {str(e)}")
            return str(e), "An error occurred. Could you please try again?"
