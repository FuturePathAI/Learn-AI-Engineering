...
from mercury_ml.chatbot.apis.shopagain import fetch_order_info, fetch_product_info, fetch_order_info_by_user
...
class OrderServiceFunctions:
    @staticmethod
    def get_order_status(company_id: str, 
        ...
        try:
            ...
            if order_info and 'order_status' in order_info:
                return format_order_status_string(order_id, email, mobile, order_info), ""
            else:
    ...
    @staticmethod
    def manage_order_change_request(company_id: str, 
                                 order_id: Optional[int] = None, 
                                 email: Optional[str] = None, 
                                 mobile: Optional[str] = None, ecom_chatbot = None) -> Tuple[str, str]:
        try:
            if order_id is not None:
                order_info = fetch_order_info(company_id, str(order_id))
            elif email is not None:
                ...
                return "", "Could you please provide the Order ID, email, or phone number?" + call_human_agent(ecom_chatbot)
    
            if order_info and 'order_status' in order_info:
                return format_order_status_string(order_id, email, mobile, order_info), "" + call_human_agent(ecom_chatbot)
            else:
                return "Order not found.", "Could you please check the provided information and try again?" + call_human_agent(ecom_chatbot)
        except Exception as e:
            logging.error(f"Exception occurred: {str(e)}")
            return str(e), "An error occurred. Could you please try again?" + call_human_agent(ecom_chatbot)
    
    @staticmethod
    def get_last_order_date(company_id: str, 
                            order_id: Optional[int] = None, 
                            email: Optional[str] = None, 
                            mobile: Optional[str] = None) -> Tuple[str, str]:
        try:
    ...
    @staticmethod
    def get_recent_order_products(company_id: str, 
                                  order_id: Optional[int] = None, 
                                  email: Optional[str] = None, 
                                  mobile: Optional[str] = None) -> Tuple[str, str]:
            ...
            if 'external_product_ids' in order_info:
                ...
                for product_id in external_product_ids:
                    ...
                    if 'name' in product_info:
                        formatted_products.append(f"Product Name: {product_info['name']}")
                
                if formatted_products:
                    formatted_products_str = '\n'.join(formatted_products)
                    return f"In your most recent order with Order ID {last_order_id}, you ordered the following products:\n{formatted_products_str}", ""
                else:
                    return "No products found in the most recent order.", "Could you please check the provided information and try again?"
