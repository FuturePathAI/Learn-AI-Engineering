...
class RAGLlamaIndex():
    ...
    def setup(self):
        # Load documents
        self.documents = SimpleDirectoryReader(input_dir=self.input_dir, exclude_hidden=False).load_data()
        ...
        if self.index is None:
            try:
                ...
                print(f"Exception occurred while loading existing index: {str(e)}. Training new index.")
    ...
    def initialize_query_engine(self):
        ...
        self.query_engine = RetrieverQueryEngine(
            ...
            node_postprocessors=[
                    ...
                    SentenceEmbeddingOptimizer(embed_model=self.service_context.embed_model,percentile_cutoff=0.5), # threshold_cutoff=0.7)
                ],
        )
    ...
    def get_response_context_with_scores(self, query: str):
        ...
        return [(node_with_score.node.text, node_with_score.score) for node_with_score in response.source_nodes]
